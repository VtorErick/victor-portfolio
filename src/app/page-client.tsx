...
export default function HomeClient({ postsEs, postsEn }: HomeClientProps) {
  ...
  const rawPhone = portfolioData.contact.phone?.trim()
            ...
            >
              ...
              <div className="flex flex-col gap-4 items-center md:items-start">
                ...
                {phone && phoneLink && (
                  <a
                    href={`tel:${phoneLink}`}
                    className="flex items-center gap-3 text-[var(--foreground)]/80 hover:text-[var(--link)] transition-colors"
                    aria-label={`${t(uiTranslations.aria.callPhone, language)} ${phone}`}
                  >
                    <div className="w-10 h-10 rounded-full bg-[var(--foreground)]/5 flex items-center justify-center">
